      ...
      <v-card elevation="10" outlined width="100%" class="mx-auto">
            ...
            <v-col  cols="12" md="9" style="margin-left:60px; margin-top:30px; ">
              <div>
                <v-text-field
                  ...
                  label="검색어"
                  single-line
                  @keypress.enter.prevent="OldBoardViewListarticlessearch"
                ></v-text-field>
              </div>
            </v-col>
          ...
          <v-row>
            <v-col>
                ...
                <div v-if="searchfinish===false">
                  <BoardViewListItem
                    v-for="(Boardarticle, index) in OldBoardViewListarticles.content"
                    :key="Boardarticle.postId"
                    :Boardarticle="Boardarticle"
                    :index="index"
                    :limit="OldBoardViewListPage"
                    @delete-Board="getOldBoardViewListArticles"
        ...
        <div v-if="OldBoardViewListarticles">
          <div style="margin: auto;" >
            <v-pagination
              v-model="this.OldBoardViewListPage"
              :length="5"
            ></v-pagination>
          </div>
        </div>
...
export default {
  name: 'OldBoardViewList',
  data() {
    return {
      OldBoardViewListPage: 1,
      OldBoardViewListarticles: [], 
      schVal: '',
      searchfinish: false,
      contentlist: [], 
    }
  ...
  created() {
    this.getOldBoardViewListArticles()

  },
  methods: {
    getOldBoardViewListArticles() {
      axios({
        method: 'get',
        url: `${VUE_APP_API_URL}/api/trouble/list/elder`,
        headers: {
          "Authorization" : `Bearer ${this.$store.state.token.token.access_token}`}
      })
        .then((res) => {
          console.log('이거 되라 게시판 전체 목록')
          this.OldBoardViewListarticles = res.data
        })
        .catch((err) => {
          console.log('어림도 없지 게시판 전체 목록')
        })
    },
    OldBoardViewListarticlessearch() {
      axios({
        method: 'get',
        url: `${VUE_APP_API_URL}/api/trouble/list/elder?searchword=${this.schVal}`
      })
        .then((res) => {
          console.log(res)
